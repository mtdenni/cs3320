...
function validate() {
    ...
    if (parseInt(userYear) >= currentYear) {    // Make sure the current year and user year ar >=
        valid.push(true);

        if (parseInt(userYear) == currentYear && validYear) {   // Ensure that if the current year is the same 
    ...
    if (valid.every(Boolean)) { // Make sure everything is valid.
        return true;
    }
    else {
        valid = [];
